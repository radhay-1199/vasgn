...
public class Util {
	...
	private Logger logger = (Logger) LogManager.getLogger(Util.class);
	...
	public HashMap<String,List<PanelContentModal>> getCategoryWiseContent(List<PanelContentModal> list)
	{
			
		HashMap<String,List<PanelContentModal>> cat= new HashMap<>();
		for(int i=0;i<list.size();i++) {
			PanelContentModal panel=list.get(i);
			String category=panel.getCategoryname();
			if(cat.containsKey(category))
			{	
				if(cat.get(category).size()<3) {
				cat.get(category).add(panel);
				}
	  		}
			else
			{
				...
				newcat.add(panel);
				cat.put(category,newcat);
			}
	 }
		System.out.println("HashMap"+cat);
		return cat;
	}
